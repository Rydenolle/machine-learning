...
protected:
    ...
    template <size_t ValueCount>
    void copy(const T (&values)[ValueCount], const size_t offset = 0U) noexcept;
    
    template <size_t ValueCount>
    void copy(const Array<T, ValueCount>& other, const size_t offset = 0U) noexcept;

    /** Static field holding data. */
    T myData[Size];
};
} // namespace container
